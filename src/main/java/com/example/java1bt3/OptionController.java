...
public class OptionController implements Initializable{
    ...
    protected void onOKButtonClick(ActionEvent event) {
        ...
        String selecteColor = (String) cbb.getValue();
        if (getValueShape() == 1) {
            Rectangle rectangle = drawRec(selecteColor);
            this.borderPane.setCenter(rectangle);
        }else if (getValueShape() == 2) {
            Ellipse ellipse = drawEllipse(selecteColor);
            this.borderPane.setCenter(ellipse);
        }else  if (getValueShape() == 3) {
            Polygon triangle = drawTriangle(selecteColor);
            this.borderPane.setCenter(triangle);
        }
    }
